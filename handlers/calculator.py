...
async def calc_compute(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=calc_delivery(back_target="calc_cat"), parse_mode="HTML"
    )
    await callback.answer()
...
async def manual_price_received(message: Message, state: FSMContext):
    ...
    await message.answer(
        texts.CALC_CHOOSE_DELIVERY,
        reply_markup=calc_delivery(back_target="calc"), parse_mode="HTML"
    )


# ── РЕЗУЛЬТАТ ──────────────────────────────────────────────────

DELIVERY_MAP = {"delivery_air": "air", "delivery_auto": "auto", "delivery_rail": "rail"}
